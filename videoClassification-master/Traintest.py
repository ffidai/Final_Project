...
model_test = Model(inputs=baseModel.input, outputs=headModel)
...
H = model_test.fit_generator(	trainAug.flow(trainX, trainY, batch_size=32), 	steps_per_epoch=len(trainX) // 32, 	validation_data=valAug.flow(testX, testY), 	validation_steps=len(testX) // 32, epochs=args["epochs"])
	#epochs=args["epochs"])
    

# evaluate the network
print("[INFO] evaluating network...")
y_score = model_test.predict(testX, batch_size=32)
